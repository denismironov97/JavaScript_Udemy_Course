...
function challengeEight() {
  ...
  const tipsArr = bills.map((bill) => {
    ...
    const currTotalValue = bill + tipValue;
    totals.push(currTotalValue);

    return tipValue;
  });

  console.log(`Bills: ${bills.join(', ')}`);
  console.log(`Tips: ${tipsArr.join(', ')}`);
  console.log(`Totals: ${totals.join(', ')}`);
  console.log(`Average price is ${calcAverage(totals).toFixed(2)}`);

  function calcAverage(arrArg) {
    let sum = 0;
    let index = 0;
    while (index < arrArg.length) {
      sum += arrArg[index];
      index++;
    }

    return sum / arrArg.length;
  }
}
challengeEight();

function challengeEightV1() {
